...
@section('page-title', 'service-List || Admin')
@push('page-css')
    <!-- DataTables -->
                    ...
                    <div class="page-title-box">
                        <div class="d-flex justify-content-between align-items-center text-muted">
                            <h4 class="page-title float-left">
                                <i class="zmdi zmdi-serviceger"></i> Service
                            </h4>
                            ...
                            <tbody>
                                @foreach ($services as $key => $service)
                                    <tr>
                                        ...
                                        <td>{{ Str::words($service->name, 10, '[..]') }}</td>
                                        <td>
                                            <img src="{{ $service->image != null ? asset($service->image) : asset('assets/application-default/img/gallery.jpg') }}"
                                                alt="" style="width: 80px;">

                                        </td>
                                        <td>
                                            <span
                                                class="badge {{ $service->status == true ? 'badge-info' : 'badge-danger' }}">{{ $service->status == true ? 'Active' : 'Inactive' }}</span>
                                        </td>
                                        <td class="btn-group action-button">
                                            ...
                                            <button type="button" onclick="deleteData({{ $service->id }})"
