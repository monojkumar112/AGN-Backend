...
@section('page-title', 'company-List || Admin')
@push('page-css')
    <!-- DataTables -->
                    ...
                    <div class="page-title-box">
                        <div class="d-flex justify-content-between align-items-center text-muted">
                            <h4 class="page-title float-left">
                                <i class="zmdi zmdi-companyger"></i> Company Logo
                            </h4>
                            <ol class="breadcrumb float-right">
                                <li class="breadcrumb-item">
                                    <a href="{{ route('admin.company.create') }}" class="btn btn-dark">
                                        <i class="fa fa-plus-circle"></i>
                                        <span>Create Company Logo</span>
                                    </a>
                                </li>
                            </ol>
                        </div>
                            ...
                            <tbody>
                                @foreach ($companys as $key => $company)
                                    <tr>
                                        ...
                                        <td>{{ Str::words($company->name, 10, '[..]') }}</td>
                                        <td>
                                            <img src="{{ $company->image != null ? asset($company->image) : asset('assets/application-default/img/gallery.jpg') }}"
                                                alt="" style="width: 80px;">

                                        </td>
                                        <td>
                                            <span
                                                class="badge {{ $company->status == true ? 'badge-info' : 'badge-danger' }}">{{ $company->status == true ? 'Active' : 'Inactive' }}</span>
                                        </td>
                                        <td class="btn-group action-button">
                                            ...
                                            <button type="button" onclick="deleteData({{ $company->id }})"
                                                class="btn btn-danger rounded-right btn-sm">
                                                <i class="fa fa-trash-o"></i>
                                            </button>
                                            <form id="delete-form-{{ $company->id }}"
                                                action="{{ route('admin.company.destroy', $company->id) }}" method="POST"
                                                style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
