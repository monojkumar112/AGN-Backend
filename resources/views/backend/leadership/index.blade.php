...
@section('page-title', 'leadership-List || Admin')
@push('page-css')
    <!-- DataTables -->
                    ...
                    <div class="page-title-box">
                        <div class="d-flex justify-content-between align-items-center text-muted">
                            <h4 class="page-title float-left">
                                <i class="zmdi zmdi-leadershipger"></i> leadership Team
                            </h4>
                            <ol class="breadcrumb float-right">
                                <li class="breadcrumb-item">
                                    <a href="{{ route('admin.leadership.create') }}" class="btn btn-dark">
                                        <i class="fa fa-plus-circle"></i>
                                        <span>Create leadership</span>
                                    </a>
                                </li>
                            </ol>
                        </div>
                            ...
                            <tbody>
                                @foreach ($leaderships as $key => $leadership)
                                    <tr>
                                        ...
                                        <td>
                                            <img src="{{ $leadership->image != null ? asset($leadership->image) : asset('assets/application-default/img/gallery.jpg') }}"
                                                alt="" style="width: 80px;">

                                        </td>
                                        <td>
                                            <span
                                                class="badge {{ $leadership->status == true ? 'badge-info' : 'badge-danger' }}">{{ $leadership->status == true ? 'Active' : 'Inactive' }}</span>
                                        </td>
                                        <td class="btn-group action-button">
                                            <a href="{{ route('admin.leadership.edit', $leadership->id) }}"
                                                class="btn btn-dark btn-sm">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            <button type="button" onclick="deleteData({{ $leadership->id }})"
                                                class="btn btn-danger rounded-right btn-sm">
                                                <i class="fa fa-trash-o"></i>
                                            </button>
                                            <form id="delete-form-{{ $leadership->id }}"
                                                action="{{ route('admin.leadership.destroy', $leadership->id) }}"
    ...
    <script type="text/javascript">
        function deleteData(id) {
            swal({
                ...
                if (result.value) {
                    leadership.prleadershipDefault();
