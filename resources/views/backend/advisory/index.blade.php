...
@section('page-title', 'advisory-List || Admin')
@push('page-css')
    <!-- DataTables -->
                    ...
                    <div class="page-title-box">
                        <div class="d-flex justify-content-between align-items-center text-muted">
                            <h4 class="page-title float-left">
                                <i class="zmdi zmdi-advisoryger"></i> Advisory Team
                            </h4>
                            <ol class="breadcrumb float-right">
                                <li class="breadcrumb-item">
                                    <a href="{{ route('admin.advisory.create') }}" class="btn btn-dark">
                            ...
                            <tbody>
                                @foreach ($advisorys as $key => $advisory)
                                    <tr>
                                        ...
                                        <td>
                                            <img src="{{ $advisory->image != null ? asset($advisory->image) : asset('assets/application-default/img/gallery.jpg') }}"
                                                alt="" style="width: 80px;">

                                        </td>
                                        <td>
                                            <span
                                                class="badge {{ $advisory->status == true ? 'badge-info' : 'badge-danger' }}">{{ $advisory->status == true ? 'Active' : 'Inactive' }}</span>
                                        </td>
                                        <td class="btn-group action-button">
                                            <a href="{{ route('admin.advisory.edit', $advisory->id) }}"
                                                class="btn btn-dark btn-sm">
                                                <i class="fa fa-edit"></i>
                                            </a>
                                            <button type="button" onclick="deleteData({{ $advisory->id }})"
                                                class="btn btn-danger rounded-right btn-sm">
                                                <i class="fa fa-trash-o"></i>
                                            </button>
                                            <form id="delete-form-{{ $advisory->id }}"
                                                action="{{ route('admin.advisory.destroy', $advisory->id) }}"
